...
sales = sales3  # keeping a backuph
...
#Ans3a: Part Num- 764821000 was purchased max times -   12
...
#(practise it )
sales %>% group_by (partnum) %>% summarise(Revenue= sum(revenue)) %>% arrange(desc(Revenue)) %>% head()
